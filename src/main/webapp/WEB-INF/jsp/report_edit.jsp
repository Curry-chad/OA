...
<div style="padding:10px 10px 10px 10px">
	<form id="reportEditForm" class="orderForm" method="post">
	    ...
	    <table cellpadding="5">
	        ...
	        <tr>
	            ...
	            <td>
	                 ...
	                 <div id="reportEditFileUploader">上传文件</div>
	                 <input type="hidden" id="reportEditFile" name="reportFile"/>
	            </td>
	        </tr>
	        <tr>
	            <td>报表备注:</td>
	...
	<div style="padding:5px">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitReportEditForm()">提交</a>
	</div>
</div>
<script type="text/javascript">
	...
	$(function(){
		//实例化富文本编辑器
		orderEditEditor = TAOTAO.createEditor("#reportEditForm [name=reportNote]");
	});
	//同步kindeditor中的内容
	orderEditEditor.sync();
	
	function submitReportEditForm(){
		if(!$('#reportEditForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return ;
		}
		orderEditEditor.sync();
		
		$.post("report/update_all",$("#reportEditForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','修改报表成功!','info',function(){
					$("#reportEditWindow").window('close');
